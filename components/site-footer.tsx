  ...
  return (
    <footer className="mt-24 bg-mocha-panel text-heritage-sand">
      <div className="mx-auto max-w-6xl px-6 py-14">
          ...
          <nav className="grid grid-cols-2 gap-x-12 gap-y-2 text-sm">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="text-heritage-sand/75 transition-colors hover:text-white"
              >
                {link.label}
              </Link>
            ))}
        ...
        <div className="mt-12 flex flex-col gap-2 border-t border-heritage-sand/15 pt-6 text-xs text-heritage-sand/55 sm:flex-row sm:items-center sm:justify-between">
